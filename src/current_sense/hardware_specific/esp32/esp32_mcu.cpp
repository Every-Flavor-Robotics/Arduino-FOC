...
// function reading an ADC value and returning the read voltage
void* _configureADCInline(const void* driver_params, const int pinA,
                          const int pinB, const int pinC)
{
...
float _readADCVoltageLowSide(const int pin, const void* cs_params)
{
  ...
  float adc_voltage_conv =
      ((ESP32MCPWMCurrentSenseParams*)cs_params)->adc_voltage_conv;

  for (int i = 0; i < adc_pin_count[unit]; i++)
  {
    if (pin == ((ESP32MCPWMCurrentSenseParams*)cs_params)
                   ->pins[i])  // found in the buffer
...
void* _configureADCLowSide(const void* driver_params, const int pinA,
                           const int pinB, const int pinC)
{
...
void _driverSyncLowSide(void* driver_params, void* cs_params)
{
  ...
  mcpwm_unit_t mcpwm_unit =
      ((ESP32MCPWMDriverParams*)driver_params)->mcpwm_unit;

  // low-side register enable interrupt
  mcpwm_dev->int_ena.timer0_tep_int_ena =
      true;  // A PWM timer 0 TEP event will trigger this interrupt
  // high side registers enable interrupt
  // mcpwm_dev->int_ena.timer0_tep_int_ena = true;//A PWM timer 0 TEZ event will
  // trigger this interrupt

  // register interrupts (mcpwm number, interrupt handler, handler argument =
  // NULL, interrupt signal/flag, return handler = NULL)
  if (mcpwm_unit == MCPWM_UNIT_0)
    mcpwm_isr_register(mcpwm_unit, mcpwm0_isr_handler, NULL, ESP_INTR_FLAG_IRAM,
                       NULL);  // Set ISR Handler
  else
    mcpwm_isr_register(mcpwm_unit, mcpwm1_isr_handler, NULL, ESP_INTR_FLAG_IRAM,
                       NULL);  // Set ISR Handler
}
