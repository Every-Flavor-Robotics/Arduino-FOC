...
InlineCurrentSense::InlineCurrentSense(float _shunt_resistor, float _gain,
                                       int _pinA, int _pinB, int _pinC)
{
  pinA = _pinA;
  pinB = _pinB;
  pinC = _pinC;
...
InlineCurrentSense::InlineCurrentSense(float _mVpA, int _pinA, int _pinB,
                                       int _pinC)
{
  pinA = _pinA;
  pinB = _pinB;
...
void InlineCurrentSense::calibrateOffsets()
{
  ...
  {
    float v_a = _readADCVoltageInline(pinA, params);
    float v_b = _readADCVoltageInline(pinB, params);
    float v_c = _readADCVoltageInline(pinC, params);
    if (_isset(pinA)) offset_ia += v_a;
    if (_isset(pinB)) offset_ib += v_b;
    if (_isset(pinC)) offset_ic += v_c;

    Serial.println("Calibration: " + String(i) + " " + String(v_a) + " " +
                   String(v_b) + " " + String(v_c));
  ...
  Serial.println("Final Offsets: " + String(offset_ia) + " " +
                 String(offset_ib) + " " + String(offset_ic));
}
...
PhaseCurrent_s InlineCurrentSense::getPhaseCurrents()
{
  ...
  current.a = (!_isset(pinA)) ? 0 : (v_a - offset_ia) * gain_a;  // amps
  current.b =
      (!_isset(pinB))
          ? 0
          : (_readADCVoltageInline(pinB, params) - offset_ib) * gain_b;  // amps
  current.c = (!_isset(pinC)) ? 0 : (v_c - offset_ic) * gain_c;          // amps

  //   Serial.println(
  //       "v_a: " + String(v_a) + " v_c: " + String(v_c) +
  //       " offset_ia: " + String(offset_ia) + " offset_ib: " +
  //       String(offset_ib) + " offset_ic: " + String(offset_ic) + "current a:
  //       " + String(current.a) + " current b: " + String(current.b) + "
  //       current c: " + String(current.c));
...
// avoided returns flag 0 - fail 1 - success and nothing changed 2 - success but
// pins reconfigured 3 - success but gains inverted 4 - success but pins
